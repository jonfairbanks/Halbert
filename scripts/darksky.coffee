...
darkSkyMe = (msg, lat, lng, cb) ->
  ...
  msg.http(url)
    .get() (err, res, body) ->
      ...
      response = "_Currently:_ #{result.currently.temperature}°F and #{result.currently.summary} (Feels like #{result.currently.apparentTemperature}°) \n\n"
      response += "_Today:_ #{result.hourly.summary} \n\n"
      response += "_Coming Week:_ #{result.daily.summary}\n\n"
      cb response
